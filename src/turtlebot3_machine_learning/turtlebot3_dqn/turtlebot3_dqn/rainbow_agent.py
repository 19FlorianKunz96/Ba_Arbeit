...
class DQNAgent(Node):
    ...
    def __init__(self):       #added param stage_boost for taking weights from last stages
        ...
        self.succeed = False
        self.fail = False
        self.info = ['Dueling,Double,Same Network Architecture, HuberLoss']
    ...
    def train_model(self, terminal):
        ...
        #prediction of next_q_value list with online network to implement double dqn. therefore target ist builded
        #with target network, but with action predicted with online network


        predict_action_list = self.model.predict(next_states)
        action_index = numpy.argmax(predict_action_list,axis=1)
        ...
        for index, (current_state, action, reward, _, done) in enumerate(data_in_mini_batch):
            ...
            if not done:
                future_reward = next_qvalues_list[index, action_index[index]]
